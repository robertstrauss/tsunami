...
runinfo = Path("runinfo.yaml")
while not runinfo.is_file():
    runinfo = Path("runinfo.yaml")
    time.sleep(1)

# read it and put values into local variables
with open("runinfo.yaml", "r") as yamlfile:
    runinfodc = yaml.safe_load(yamlfile)
latstart, latend, lonstart, lonend, tsulat, tsuamp, tsutype, tsuwx, wsuwy, theta = runinfodc['latstart'], runinfodc['latend'], runinfodc['lonstart'], runinfodc['lonend'], runinfodc['tsulat'], runinfodc['tsuamp'], runinfodc['tsutype'], runinfodc['tsuwx'], runinfodc['wsuwy'], runinfodc['theta']
