...
def forward2_u_driver_py (h, n, u, v,f,  dx, dy, out_u, out_v, out_n):  
...
compiler = cuda.jit('void(float32[:,:], float32[:,:], float32[:,:],float32[:,:],float32[:,:],float32,float32,float32[:,:],float32[:,:],float32[:,:])' ,inline=True,fastmath=True)        
...
def inner(ii,jj,i,j,h, n, u, v, f, dx, dy, out_u, out_v, out_n):
...
def cu_u_driver_global_py(h, n, u, v, f, dx, dy, out_u, out_v, out_n):
...
def cu_u_driver_global_inner_py(h, n, u, v, f, dx, dy, out_u, out_v, out_n):
    sj,si  = cuda.grid(2)  # cuda.x cuda.y skow,fast
    gj,gi = sj,si
    if (gi < out_u.shape[0]-1) and (gj < out_u.shape[1]-1):
        if  gi>0  and gj>0 : 
            inner(gi,gj,si,sj,h, n, u, v, f, dx, dy, out_u, out_v, out_n)

def ufuncs(si,sj,h, n, f, u, v, dx, dy):
            tmp=np.float32(0.0)
            ...
            return tmp
def vfuncs(si,sj,h, n, f, u, v, dx, dy):
            tmp=np.float32(0.0)
            tmp +=  cuda_dvdt2_drag_ret(si,sj,h, n, f, u, v, dx, dy)  
            tmp += cuda_dvdt2_advection_ret(si,sj,h, n, f, u, v, dx, dy)  
            tmp += cuda_dvdt2_grav_ret(si,sj,h, n, f, u, v, dx, dy) 
            tmp += cuda_dvdt2_coriolis_ret(si,sj,h, n, f, u, v, dx, dy)
            return tmp
def cu_u_driver_global_funcs_py(h, n, u, v, f, dx, dy, out_u, out_v, out_n):
  
    sj,si  = cuda.grid(2)  # cuda.x cuda.y skow,fast
    gj,gi = sj,si
    if (gi < out_u.shape[0]-1) and (gj < out_u.shape[1]-1):
        if  gi>0  and gj>0 : 
            out_u[gi,gj]=ufuncs(si,sj,h, n, f, u, v, dx, dy)
            out_v[gi,gj]=vfuncs(si,sj,h, n, f, u, v, dx, dy)
# 
#             tmp=np.float32(0.0)
#             tmp +=  cuda_dudt2_drag_ret(si,sj,h, n, f, u, v, dx, dy)  
#             tmp += cuda_dudt2_advection_ret(si,sj,h, n, f, u, v, dx, dy)  
...
def cu_u_driver_global_inner_col_py(h, n, u, v, f, dx, dy, out_u, out_v, out_n):
    sj,si  = cuda.grid(2)  # cuda.x cuda.y skow,fast
    gj,gi = sj,si
...
def cu_u_driver_global_funcs_col_py(h, n, u, v, f, dx, dy, out_u, out_v, out_n):
    sj,si  = cuda.grid(2)  # cuda.x cuda.y skow,fast
    gj = sj
...
#@cuda.jit('void(float32[:,:], float32[:,:], float32[:,:],float32[:,:],float32[:,:],float32,float32,float32[:,:])')
def cu_u_driver_shared_py(uh, un, uu, uv, uf, dx, dy, out_u, out_v, out_n):
    """This kernel function will be executed by a thread."""
    ii,jj  = cuda.grid(2)   #slow,fast    # in our system i is slow/far and j is fast/near,  usually defined differently
                                          # cuda  y is slow and cuda  x is fast  (this is also flipped from our system)
                                          #  in our system    i(x) --> cuda.y  and j(y) --> cuda.x
...
device_ret_compiler = cuda.jit(  'void(int32,int32,int32,int32,float32[:,:], float32[:,:], float32[:,:],float32[:,:],float32[:,:],float32,float32,float32[:,:],float32[:,:],float32[:,:])',device=True,fastmath=True) 
...
ufuncs = device_ii_ret_compiler(ufuncs)
vfuncs = device_ii_ret_compiler(vfuncs)
...
def cu_u_driver_shared_inner_py(uh, un, uu, uv, uf, dx, dy, out_u, out_v, out_n):
    """This kernel function will be executed by a thread."""
...
#    j = jj
    ...
    if (ii < out_u.shape[1]-1) and (jj < out_u.shape[0]):
        ...
        if  i>0  and j>0  and i< by-2 and j<bx-2 : #####################################################
           ...
           inner(ii,jj,i,j,h, n, u, v, f, dx, dy, out_u, out_v, out_n)
   # cuda.syncthreads()  # maybe not needed
...
def cuda_u_driver_shared_py(uh, un, uu, uv, uf, dx, dy, out_u, out_v, out_n):
    """This kernel function will be executed by a thread."""
...
def cuda_u_driver_shared_inner_py(uh, un, uu, uv, uf, dx, dy, out_u, out_v, out_n):
    """This kernel function will be executed by a thread."""
...
def testin():
    ...
    out_n = np.asarray(np.random.random(n.shape) ,dtype=np.float32)
    ...
    def testit(blockspergrid,threadsperblock,cu_u_driver,res1=None ):
                ...
                out_v1 =cuda.to_device(out_v)
                out_n1 =cuda.to_device(out_n)
                ...
                for i in range(REPS):
                    ...
                    for j in range(ITRS):   
                        cu_u_driver[blockspergrid,threadsperblock](h1, n1, u1, v1, f1, dx, dy, out_u1, out_v1, out_n1)
                        cu_u_driver[blockspergrid,threadsperblock](h1, n1, u1, v1, f1, dx, dy, out_u1, out_v1, out_n1)
                        cu_u_driver[blockspergrid,threadsperblock](h1, n1, u1, v1, f1, dx, dy, out_u1, out_v1, out_n1)
...
#             
   ...
   # %timeit   orig_v = dudt(h, n, f, u, v, dx, dy, nu )
    ...
    for i in range(REPS):
        t = mytime()  # time.process_time()
        for j in range(ITRS):
            forward2_u_driver(h, n, u, v, f, dx, dy, out_u, out_v, out_n)
            forward2_u_driver(h, n, u, v, f, dx, dy, out_u, out_v, out_n)
            forward2_u_driver(h, n, u, v, f, dx, dy, out_u, out_v, out_n)
        t2 =  mytime()  # time.process_time()
        ts.append(t-t2)
