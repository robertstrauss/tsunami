...
#    j = jj
    ...
    if (ii < out_u.shape[0]-2) and (jj < out_u.shape[1]-1):
        k = np.float32(0.0)
...
#    j = jj
    ...
    if (ii < out_u.shape[0]-1) and (jj < out_u.shape[1]):
        k = np.float32(0.0)
...
def inner(ii,jj,i,j,h, n, u, v, f, dx, dy, out_u):
            ...
            for mm in range(1): 
              if  i>0  and j>0  and i< by-2 and j<bx-2 :   #  the edges are a problem here.   
                last_v_n_i = (vm1m0+vm1p1)*p5
                v_n = (vm0m0+vm0p1)*p5  # stradles n[i,j]
                    # advection
                # malfunctions when i=0 or i=n
...
def cuda_load_shared(gi,gj,si,sj,su,uu):
    # this loads everything but the corners.
    # the reason we don't load these it it requires a threadsync and 
    # we want to minimize that.

    if gi < uu.shape[1] and gj<uu.shape[0]:
        su[sj,si] = uu[gj,gi]

        cuda.threadfence()  # assure that su[sj,0] = su[sj,1] is valid
        # copy row along zero-col
        if si==1  and gi>0 :
            su[sj,0] = uu[gj,gi-1]

          #  su[sj,0] = un[gj,gi-1]
        else:
            su[sj,0] = su[sj,1]  # just copy the neighbor 

        # copy column along zero-row   
        if sj==1  and gj>0 :  #and i!=0
            su[0,si] = uu[gj-1,gi]

        else:
            su[0,si] = su[1,si]  # just copy the neighbor 

            
        if si==TPBX+1 and gi < uu.shape[1]-1:  # last thread
            su[sj,-1] = uu[gj,gi+1]
        else:
            su[sj,si+1] = su[sj,si]  # just copy the neighbor 
 

        if sj==TPBY+1 and gj<uu.shape[0]-1 : # last thread
            su[-1,si] = uu[gj+1,gi]
        else:
            su[sj+1,si] = su[sj,si]  # just copy the neighbor 
    else:  # partial block  # DO I NEED A THREAD SYNC?
        if gi == uu.shape[1] and gj<uu.shape[0]:
            su[sj,si] = su[sj,si-1]  # just copy the neighbor 
        if gj == uu.shape[0] and gi<uu.shape[1]:    
            su[sj,si] = su[sj-1,si]  # just copy the neighbor 

@cuda.jit('void(int32,int32,int32,int32,float32[:,:],float32[:,:])',device=True)  
def cuda_load_shared_corners(gi,gj,si,sj,su,uu):
        #  THIS IS WRONG!   IT WORKS FOR FULL BLOCKS BUT FOR
        #  PARTIAL BLOCKS THE CORNERS ARE IN A DIFFERENT PLAC
        
                        # now do the corners.  Might not actually require this.
        ...
        if si==1:
            if  sj==1:   
                if gi>0 and gj >0:
                    ...
                    su[0,0] = (su[1,0]+su[1,1]+su[0,1])/np.float32(3.0)
            if sj==TPBY+1:
                if gi>0 and gj<uu.shape[0]-1:
                    su[-1,0] = uu[gj+1,gi-1] 
                else:
                    su[-1,0] = (su[-2,0]+su[-2,1]+su[-1,0])/np.float32(3.0)

            
        if si==TPBX+1:
            if  sj==1:    # technicall after the threadsync we don't care which thres does this op.
                if gi<uu.shape[1]-1 and gj >0:
                    su[0,-1] = uu[gj-1,gi+1] 
                else: 
                    su[0,-1] = (su[1,-2]+su[1,-1]+su[0,-1])/np.float32(3.0)  # check this
            if sj==TPBY+1:
                if gi<uu.shape[1]-1 and gj<uu.shape[0]-1:
                    su[-1,-1] = uu[gj+1,gi+1] 
                else:
                    su[-1,-1] = (su[-2,-1]+su[-2,-2]+su[-1,-2])/np.float32(3.0)
...
#     sj = cuda.threadIdx.y + 1; # local j for shared memory access
  ...
  #  This limits the maximum block size to 512.
    ...
    #    tmp += cuda_dvdt2_coriolis_ret(sj,si,h, n, f, u, v, dx, dy)  
        out_u[gj,gi] =tmp  ##  IS THIS REGISTERED RIGHT??  SHOULD IT be gj+1, gi+1:  I think so because si and sj are restricted.
...
#        cuda_load_shared_corners(gi,gj,si,sj,sn,un)
    ...
    # discard edge threads and truncate at boundaries
    if TPBX2-1>si>0 and TPBY2-1>sj>0 and gi<out_u.shape[1] and gj<out_u.shape[0] :
        inner(gj,gi,sj,si,h, n, u, v, f, dx, dy, out_u)
...
#     print("threads per block",threadsperblock)
    ...
    try:
        for cu_u_driver in (cuda_u_driver_shared_inner,cuda_u_driver_shared,cu_u_driver_global,cu_u_driver_shared_inner,cu_u_driver_shared,):
...
#                     cu_u_driver[blockspergrid,threadsperblock](h1, n1, u1, v1, f1, dx, dy, out_u1)
            ...
            print (ts)
            result  = out_u1[segment[0]:segment[1],segment[0]:segment[1]].copy_to_host()
            if i==0:  res1 = result
            print (result)
            result -= res1
            result = result[-2:-2,-2:2]
            print (np.max(res),np.min(result),np.std(result))
    finally:
        print ("cuda closer")
        cuda.close()  
    print ("all done")
